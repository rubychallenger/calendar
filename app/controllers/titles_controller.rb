class TitlesController < ApplicationController
  ...
  def refresh_API
    ...
    series_response.each_with_index do |ep,index|
      episode = Episode.create_with(airdate: DateTime.strptime( (ep["FirstAired"] || series_respons[index-1]["FirstAired"]) + " " + full_response["Series"]["Airs_Time"], '%Y-%m-%d %I:%M %p' )).find_or_create_by(name: ep["EpisodeName"]) 
